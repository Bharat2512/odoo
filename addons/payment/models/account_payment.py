...
class AccountPayment(models.Model):
    ...
    def onchange_partner_id(self):
        res = {}
        if self.partner_id:
            res['domain'] = {'electronic_payment_token_id': [('partner_id', '=', self.partner_id.id)]}
    ...
    def _clear_electronic_payment_token(self):
        if self.payment_method_id.code == 'electronic':
            self.electronic_payment_token_id = self.env['payment.token'].search([('partner_id', '=', self.partner_id.id)], limit=1)
        else:
    ...
    def _do_payment(self):
        ...
        tx_values = {
            'amount': self.amount,
            'acquirer_id': self.electronic_payment_token_id.acquirer_id.id,
            'type': 'server2server',
            'currency_id': self.currency_id.id,
            'reference': reference,
            'payment_token_id': self.electronic_payment_token_id.id,
    ...
    def create(self, vals):
        ...
        if res.electronic_payment_token_id:
            res._do_payment()
